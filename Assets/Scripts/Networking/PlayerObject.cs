...
public class PlayerObject : NetworkBehaviour
{
	...
	// User Settings
	[Networked(OnChanged = nameof(StatChanged))]
	...
	[Networked(OnChanged = nameof(SpectatorChanged))]
	public bool IsSpectator { get; set; }
	[Networked]
	public int Strokes { get; set; }
	[Networked(OnChanged = nameof(TimeTakenChanged))]
	...
	static void SpectatorChanged(Changed<PlayerObject> p)
	{
		p.Behaviour.OnSpectatorChanged?.Invoke();
	}

	static void TimeTakenChanged(Changed<PlayerObject> p)
	{
		if (p.Behaviour.TimeTaken != TIME_UNSET)
		{
			if (p.Behaviour.Object.HasInputAuthority)
			{
				HUD.ForceHideAll();
			}
